...
class Application:
    ...
    def select_browser(self) -> 'ExitCode':
        ...
        process = self.make_menu('', f"Your current browser is {self.browser.get_name()}.", (b.get_name() for b in browsers))
        if process.returncode == 0:
            browser_name = process.stdout.decode(DEFAULT_ENCODING).strip()
            self.browser = Browser.all[browser_name]
            return ExitCode.SUCCESS
    ...
    def select_search_engine(self) -> 'ExitCode':
        ...
        process = self.make_menu('󰖟', f"Your current search engine is {self.search_engine.get_name()}.", (se.get_name() for se in search_engines))
        if process.returncode == 0:
            search_engine_name = process.stdout.decode(DEFAULT_ENCODING).strip()
            self.search_engine = SearchEngine.all[search_engine_name]
            return ExitCode.SUCCESS
    ...
    def run(self) -> 'ExitCode':
        ...
        terms = process.stdout.decode(DEFAULT_ENCODING).strip()
        if process.returncode == 0:
            url = self.search_engine.search(terms, self.language)
